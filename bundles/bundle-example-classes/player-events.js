...
import { Broadcast as B, Logger, SkillErrors } from '@friday/ranvier';
import Combat from '../bundle-example-combat/lib/Combat';
import CombatErrors from '../bundle-example-combat/lib/CombatErrors';
                ...
                try {
                    ...
                    if (e instanceof SkillErrors.CooldownError) {
                        if (ability.cooldownGroup) {
                            return B.sayAt(
                                this,
                                `Cannot use ${ability.name} while ${e.effect.skill.name} is on cooldown.`,
                            );
                        }
                        return B.sayAt(
                            this,
                            `${ability.name} is on cooldown. ${humanize(e.effect.remaining)} remaining.`,
                        );
                    }
