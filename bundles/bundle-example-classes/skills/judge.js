...
import { Broadcast, Damage, Heal, SkillType } from '@friday/ranvier';
    ...
    run: (state) =>
        function (args, player, target) {
            ...
            Broadcast.sayAtExcept(
                player.room,
                `<b><yellow>${player.name} conjures concentrated holy energy and slams it into ${target.name}!</yellow></b>`,
                [target, player],
            );
            Broadcast.sayAt(
                target,
                `<b><yellow>${player.name} conjures concentrated holy energy and slams it into you!</yellow></b>`,
            );
