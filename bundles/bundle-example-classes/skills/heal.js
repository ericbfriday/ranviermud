...
import { Broadcast as B, Heal, SkillType } from '@friday/ranvier';
    ...
    run: (state) =>
        function (args, player, target) {
            ...
            if (target !== player) {
                ...
                B.sayAtExcept(
                    player.room,
                    `<b>${player.name} calls upon to the light to heal ${target.name}'s wounds.</b>`,
                    [target, player],
                );
                ...
                B.sayAtExcept(player.room, `<b>${player.name} calls upon to the light to heal their wounds.</b>`, [
                    player,
                    target,
                ]);
            }
