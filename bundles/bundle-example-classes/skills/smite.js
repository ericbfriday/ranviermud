...
import { Broadcast, Damage } from '@friday/ranvier';
...
export default {
    ...
    run: (state) =>
        function (args, player, target) {
            ...
            Broadcast.sayAt(
                player,
                `<b><yellow>Your weapon radiates holy energy and you strike ${target.name}!</yellow></b>`,
            );
            Broadcast.sayAtExcept(
                player.room,
                `<b><yellow>${player.name}'s weapon radiates holy energy and they strike ${target.name}!</yellow></b>`,
                [target, player],
            );
            Broadcast.sayAt(
                target,
                `<b><yellow>${player.name}'s weapon radiates holy energy and they strike you!</yellow></b>`,
            );
