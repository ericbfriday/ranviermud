...
import { Broadcast as B, CommandManager } from '@friday/ranvier';
...
subcommands.add({
    ...
    command: (state) => (args, player) => {
        ...
        const target = parseDot(args, player.room.players);
        ...
        say(
            target,
            `<b><green>${player.name} invited you to join their group. Join/decline with '<white>group join/decline ${player.name}</white>'</green></b>`,
        );
