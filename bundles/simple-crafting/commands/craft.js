...
import Crafting from '../lib/Crafting';
const say = B.sayAt;
import { display, renderItem } from '../../bundle-example-lib/lib/ItemUtil';
    ...
    command: (state) => (args, player) => {
        ...
        if (!itemNumber) {
            ...
            return category.items.forEach((categoryEntry, index) => {
                say(player, sprintf('%2d) ', index + 1) + display(categoryEntry.item));
            });
        }
        ...
        for (const [resource, amount] of Object.entries(item.recipe)) {
            ...
            say(player, `  ${display(ingredient)} x ${amount}`);
        }
    },
});
    ...
    command: (state) => (args, player) => {
        ...
        for (const [resource, recipeRequirement] of Object.entries(item.recipe)) {
            ...
            if (playerResource < recipeRequirement) {
                return say(
                    player,
                    `You don't have enough resources. 'craft list ${args}' to see recipe. You need ${recipeRequirement - playerResource
                    } more ${resource}.`,
                );
            }
        }
        ...
        for (const [resource, amount] of Object.entries(item.recipe)) {
            ...
            say(player, `<green>You spend ${amount} x ${display(resItem)}.</green>`);
        }

        state.ItemManager.add(item.item);
        player.addItem(item.item);
        say(player, `<b><green>You create: ${display(item.item)}.</green></b>`);
        player.save();
    },
});

export const usage = 'craft <list/create> [category #] [item #]';
