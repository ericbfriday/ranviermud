...
import { Broadcast as B, CommandManager, ItemType } from '@friday/ranvier';
const say = B.sayAt;
import { renderItem, qualityColorize, display } from '../../bundle-example-lib/lib/ItemUtil';
import { parseDot } from '../../bundle-example-lib/lib/ArgParser';
...
subcommands.add({
    ...
    command: (state) => (vendor, args, player) => {
        ...
        if (args) {
            const item = parseDot(args, items);
            if (!item) {
            ...
            B.sayAt(
                player,
                `Cost: <b><white>[${friendlyCurrencyName(vendorItem.currency)}]</white></b> x ${vendorItem.cost}`,
            );
            return;
        }
        ...
        for (const [, itemCategory] of Object.entries(ItemType)) {
            ...
            for (const item of category.items) {
                ...
                B.sayAt(
                    player,
                    '<yellow>|</yellow> ' +
                    qualityColorize(item, sprintf('%-48s', `[${item.name}]`)) +
                    sprintf(
                        ' <yellow>|</yellow> <b>%-26s</b>',
                        B.center(26, friendlyCurrencyName(vendorItem.currency) + ' x ' + vendorItem.cost),
                    ) +
                    '<yellow>|</yellow> ',
                );
            }
...
subcommands.add({
    ...
    command: (state) => (vendor, args, player) => {
        ...
        if (!playerCurrency || playerCurrency < vendorItem.cost) {
            return tell(
                `You can't afford that, it costs ${vendorItem.cost} ${friendlyCurrencyName(vendorItem.currency)}.`,
            );
        }
        ...
        say(
            player,
            `<green>You spend <b><white>${vendorItem.cost} ${friendlyCurrencyName(vendorItem.currency)
            }</white></b> to purchase ${display(item)}.</green>`,
        );
        player.save();
    },
});
...
subcommands.add({
    ...
    command: (state) => (vendor, args, player) => {
        ...
        const item = parseDot(itemArg, player.inventory);
        if (!item) {
        ...
        say(
            player,
            `<green>You sell ${display(item)} for <b><white>${sellable.value} ${friendlyCurrencyName(sellable.currency)
            }</white></b>.</green>`,
        );
...
subcommands.add({
    ...
    command: (state) => (vendor, args, player) => {
        ...
        const targetItem = parseDot(itemArg, player.inventory);
        ...
        tell(
            `I could give you <b><white>${sellable.value} ${friendlyCurrencyName(sellable.currency)}</white></b> for ${display(targetItem)
            }.</green>`,
        );
    },
});

export const aliases = ['vendor', 'list', 'buy', 'sell', 'value', 'appraise', 'offer'];
export const usage = 'list [search], buy <item>, sell <item>, appraise <item>';
export function command(state) {
    return (args, player, arg0) => {
