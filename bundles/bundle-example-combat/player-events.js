...
import { Config, Broadcast as B } from '@friday/ranvier';
import Combat from './lib/Combat';
import CombatErrors from './lib/CombatErrors';
import LevelUtil from '../bundle-example-lib/lib/LevelUtil';
const WebsocketStream = require('../websocket-networking/lib/WebsocketStream').default;
...
function promptBuilder(promptee) {
    ...
    const formatProgressBar = (name, progress, entity) => {
        ...
        return `<b>${name}${pad}</b>: ${progress} <b>${entity.getAttribute('health')}/${entity.getMaxAttribute('health')
            }</b>`;
    };
