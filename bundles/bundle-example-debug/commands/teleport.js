...
import { Broadcast, PlayerRoles } from '@friday/ranvier';
    ...
    command: (state) => (args, player) => {
        if (!args || !args.length) {
            return Broadcast.sayAt(
                player,
                'Must specify a destination using an online player or room entity reference.',
            );
        }
        ...
        if (isRoom) {
            ...
            if (!targetRoom) {
                ...
                return Broadcast.sayAt(
                    player,
                    "You try really hard to teleport before realizing you're already at your destination.",
                );
            }
        } else {
        ...
        player.moveTo(targetRoom, () => {
            Broadcast.sayAt(
                player,
                '<b><green>You snap your finger and instantly appear in a new room.</green></b>\r\n',
            );
